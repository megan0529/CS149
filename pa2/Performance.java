...
public class Performance implements Runnable {
   ...
   public Performance(int keyRange, int readPercent, int numThreads, int testTime, IntSet impl, boolean verbose) {
   ...
   private void test() throws InterruptedException, BrokenBarrierException {
      ...
      if (verbose) {
         System.out.printf("%18s, %2d threads: %9.0f operations/sec %9.0f operations/sec/thread\n", impl.getClass().getSimpleName(), numThreads, total * 1000.0 / testTime, total * 1000.0 / testTime
                  / numThreads);
      }
   }
}
